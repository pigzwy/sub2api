...
type Group struct {
	...
	// 是否允许此 OpenAI 分组访问 /v1/realtime 语音接口
	AllowRealtime bool `json:"allow_realtime,omitempty"`
...
func (_m *Group) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case group.FieldAllowRealtime:
			if value, ok := values[i].(*sql.NullBool); !ok {
				return fmt.Errorf("unexpected type %T for field allow_realtime", values[i])
			} else if value.Valid {
				_m.AllowRealtime = value.Bool
			}
...
func (_m *Group) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("allow_realtime=")
	builder.WriteString(fmt.Sprintf("%v", _m.AllowRealtime))
	builder.WriteString(", ")
