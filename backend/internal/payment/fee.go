...
func CalculatePayAmountForCurrency(rechargeAmount float64, feeRate float64, currency string) string {
	return CalculatePayAmountDecimal(decimal.NewFromFloat(rechargeAmount), feeRate, currency)
}

// CalculatePayAmountDecimal 按币种精度计算应付金额，手续费向上取整到该币种最小支付单位。
func CalculatePayAmountDecimal(amount decimal.Decimal, feeRate float64, currency string) string {
