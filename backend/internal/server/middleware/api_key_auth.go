...
//
// /v1/usage、只读计费元数据端点与异步生图任务查询只需鉴权，不需要计费执行。
// usage 允许过期/配额耗尽的 Key 查询自身用量，计费元数据用于读取当前 Key 的倍率或
// 模型售价配置，异步生图查询允许已耗尽额度的 Key 拉取自身任务结果。
func apiKeyAuthWithSubscription(apiKeyService *service.APIKeyService, subscriptionService *service.SubscriptionService, cfg *config.Config) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		billingMetadataRequest := isAPIKeyBillingMetadataRead(c.Request.Method, c.Request.URL.Path)
		// Async image task polling only reads data that already belongs to the
		// authenticated key and must remain available after the completed
		// generation consumes the key's remaining balance.
		skipBilling := c.Request.URL.Path == "/v1/usage" || billingMetadataRequest || isAsyncImageTaskRead(c.Request.Method, c.Request.URL.Path)
		...
		// 计费元数据自省不需要订阅数据；/v1/usage 仍保留原有订阅读取行为。
		if isSubscriptionType && subscriptionService != nil && !billingMetadataRequest {
...
func isAPIKeyBillingMetadataRead(method, path string) bool {
	if method != http.MethodGet {
		return false
	}
	return path == "/v1/sub2api/billing" || path == "/v1/sub2api/model-pricing"
}
