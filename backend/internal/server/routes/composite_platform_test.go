...
func TestCompositeTargetPlatformMiddlewareResolvesRealtimeQuery(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			router.Use(gin.HandlerFunc(servermiddleware.APIKeyAuthMiddleware(func(c *gin.Context) {
				...
				c.Set(string(servermiddleware.ContextKeyAPIKey), &service.APIKey{
					...
					Group: &service.Group{
						...
						ModelAllowlist: service.GroupModelAllowlist{Enabled: true, Models: []string{tc.publicModel}},
					},
				})
				c.Next()
			})))
			...
			denied := httptest.NewRequest(http.MethodGet, tc.path+"?model=blocked-realtime", nil)
			denied.Header.Set("Connection", "Upgrade")
			denied.Header.Set("Upgrade", "websocket")
			deniedResponse := httptest.NewRecorder()
			router.ServeHTTP(deniedResponse, denied)
			require.Equal(t, http.StatusNotFound, deniedResponse.Code)
			require.Contains(t, deniedResponse.Body.String(), "not available for this group")
		})
	}
}
