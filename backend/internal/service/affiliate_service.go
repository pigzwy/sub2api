...
type AffiliateRepository interface {
	...
	AccrueFirstRechargeQuota(ctx context.Context, inviterID, inviteeUserID int64, amount float64, freezeHours int, sourceRef string) (bool, error)
...
func (s *AffiliateService) accrueInviteRebate(ctx context.Context, inviteeUserID int64, baseRechargeAmount float64, sourceRef string) (float64, error) {
	...
	var applied bool
	if sourceRef != "" {
		applied, err = s.repo.AccrueFirstRechargeQuota(ctx, *inviteeSummary.InviterID, inviteeUserID, rebate, freezeHours, sourceRef)
	} else {
