...
func (s *BillingService) computeTokenBreakdown(
	...
	// 缓存读取：先切出图片缓存 token（上游分价），再切出音频缓存 token（realtime 语音），
	// 余下按文本缓存价计算，总额仍记入 CacheReadCost。
	imageCacheReadTokens := min(max(tokens.ImageCacheReadTokens, 0), max(tokens.CacheReadTokens, 0))
	if pricing.ImageCacheReadPricePerToken <= 0 {
		imageCacheReadTokens = 0
	}
	audioCacheReadTokens := min(max(tokens.AudioCacheReadTokens, 0), max(tokens.CacheReadTokens-imageCacheReadTokens, 0))
	audioCacheReadPrice := pricing.AudioCacheReadPricePerToken
	if audioCacheReadPrice == 0 {
		audioCacheReadPrice = cacheReadPrice
	}
	bd.CacheReadCost = float64(tokens.CacheReadTokens-imageCacheReadTokens-audioCacheReadTokens)*cacheReadPrice +
		float64(imageCacheReadTokens)*pricing.ImageCacheReadPricePerToken +
