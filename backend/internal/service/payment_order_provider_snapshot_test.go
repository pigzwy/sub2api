...
func TestCreateOrderInTx_WritesProviderSnapshot(t *testing.T) {
	...
	require.Equal(t, 3, psSnapshotIntValue(order.ProviderSnapshot["schema_version"]))
	...
	require.Equal(t, "88.00", order.ProviderSnapshot["package_amount"])
	require.Equal(t, "88.00", order.ProviderSnapshot["credit_amount"])
	require.Equal(t, "88.00", order.ProviderSnapshot["pay_amount"])
	require.Equal(t, payment.TypeAlipay, order.ProviderSnapshot["payment_type"])
	require.Equal(t, payment.OrderTypeBalance, order.ProviderSnapshot["order_type"])
...
func TestBuildPaymentOrderProviderSnapshot_IncludesProviderCurrency(t *testing.T) {
	...
	infiniSnapshot := buildPaymentOrderProviderSnapshot(&payment.InstanceSelection{
		InstanceID:  "79",
