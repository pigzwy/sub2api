...
func TestCalculateCreditedBalanceUsesPackageBonusWhenConfigured(t *testing.T) {
	...
	if got != 50 {
		t.Fatalf("starter credit = %v, want 50 (explicit zero bonus)", got)
	}
	got = calculateCreditedBalance(100, 1.05, packages)
	if got != 102.99 {
		t.Fatalf("standard credit = %v, want 102.99", got)
	}
...
func TestBuildCheckoutRechargePackagesIncludesComputedCredit(t *testing.T) {
	...
	if got[0].Credit != 50 || got[0].Bonus != 0 {
		t.Fatalf("starter checkout = %+v, want credit 50 bonus 0", got[0])
	}
	if got[1].Credit != 102.99 || got[1].Bonus != 2.99 {
		t.Fatalf("standard checkout = %+v, want credit 102.99 bonus 2.99", got[1])
	}
}
