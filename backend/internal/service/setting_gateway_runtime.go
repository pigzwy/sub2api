...
// RequestAuditRuntime is the hot-path request/response audit configuration.
...
// GetRequestAuditRuntime returns request audit settings for gateway hot paths; on read errors it defaults to disabled to avoid unexpected sensitive data capture.
func (s *SettingService) GetRequestAuditRuntime(ctx context.Context) RequestAuditRuntime {
	...
	result, _, _ := s.requestAuditRuntimeSF.Do("request_audit_runtime", func() (any, error) {
		...
		if err != nil {
			...
			entry := &cachedRequestAuditRuntime{value: RequestAuditRuntime{}, expiresAt: time.Now().Add(requestAuditRuntimeErrorTTL).UnixNano()}
