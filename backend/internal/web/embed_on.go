...
func NewFrontendServer(settingsProvider PublicSettingsProvider) (*FrontendServer, error) {
	return newFrontendServer(settingsProvider, false)
}

// NewStaticFrontendServer creates a frontend server with a stable HTML shell.
// Runtime public settings are loaded by the frontend from the public settings
// endpoint instead of being embedded into index.html. This keeps the shell
// safe for short-lived edge caching and avoids reusing CSP nonces in cached
// responses.
func NewStaticFrontendServer() (*FrontendServer, error) {
	return newFrontendServer(nil, true)
}

func newFrontendServer(settingsProvider PublicSettingsProvider, staticShell bool) (*FrontendServer, error) {
	...
	return &FrontendServer{
		...
		staticETag:  StaticHTMLETag(baseHTML),
...
func (s *FrontendServer) Middleware() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		// A browser navigation is the only request that may fall back to the SPA
		// shell. Write methods must continue to the registered routes so a
		// malformed POST /login cannot receive a successful index.html response.
		if cleanPath == "index.html" || !s.fileExists(cleanPath) {
			if !isFrontendNavigationMethod(c.Request.Method) {
				c.Next()
				return
			}
			s.serveIndexHTML(c)
...
func (s *FrontendServer) serveStaticIndexHTML(c *gin.Context) {
	...
	if s.staticETag != "" {
		...
		if c.GetHeader("If-None-Match") == s.staticETag {
			c.Status(http.StatusNotModified)
			c.Abort()
			return
		}
