...
|---|---|
| 统计日期 | 2026-09-02 |
| 上游基线 | `5097b3145`（`v0.2.0`，已合并入本分支） |
| 分支共同祖先 | `5097b3145`（`v0.2.0`，本次合并后） |
| 差异规模 | 233 个文件，+25465 / -411 行 |
| 其中后端 | 171 个文件，+20875 / -214 行 |
| 其中前端 | 55 个文件，+2972 / -197 行（其余 7 个为根目录、部署和文档文件） |

合并 `v0.2.0` 后 `request-audit` 不再落后上游（`git rev-list --count HEAD..upstream/main` = 0）。

本次生产源码复核以合并后的 `request-audit` 为基线；该提交包含上游
`v0.2.0` 及本文件列出的独有功能。仓库没有可访问的 GitHub Wiki remote，
