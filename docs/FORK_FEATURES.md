...
|---|---|
| 统计日期 | 2026-09-07 |
| 上游基线 | `b7dba6267`（`v0.2.2`，已合并入本分支） |
| 分支共同祖先 | `b7dba6267`（`v0.2.2`，本次合并后） |
| 差异规模 | 234 个文件（行数以重新核对命令为准） |

合并 `v0.2.2` 后 `request-audit` 不再落后上游（`git rev-list --count HEAD..upstream/main` = 0）。

本次生产源码复核以合并后的 `request-audit` 为基线；该提交包含上游
`v0.2.2` 及本文件列出的独有功能。仓库没有可访问的 GitHub Wiki remote，
...
v0.2.2 已采用上游模型白名单：旧 `models_list_config` 被重命名为
`model_allowlist`，开启后同时限制模型列表与请求准入。以下历史章节提到的“模型列表
配置”现应按此语义理解。复合分组的 Realtime、Gemini Images 和异步媒体同样经过
白名单检查，校验客户端公开模型名后再执行复合路由；Studio 价格元数据接口保持
只读鉴权访问。部署前应补齐启用白名单分组的图片、视频和语音模型。
...
- 管理后台 `系统设置 → 网关服务 → 请求审计`：开关、保留时长、分组范围、按邮箱搜索的用户范围。与「本地直答」同组——两者都是针对网关请求的处理策略。
- 管理后台 `用量统计 → 请求审计`：开关打开后才出现的标签页，可查看请求体、响应体、状态码、耗时、账号、用户和 API Key。

**2026-08-20 位置调整**：这两张卡原本在「功能设置」标签页，邻居是模型广场、签到等无关开关，显得孤立。现统一移到「网关服务」。中途曾把审计设置整块搬进用量页的审计标签页（提交 63ce22d57，已 revert）——「看和配在一起」听着合理，但那是一个**全局功能开关**，放进纯看数据的用量页没人想得到去那里找；配套改动（标签页常驻、从 SettingsView 保存负载里摘掉这四个 key）也一并撤销了。
...
对选中分组的请求先做本地匹配，命中后由本站直接作答，不发给上游模型，因此不消耗上游额度。默认关闭；未选择分组时即使总开关打开也不作答。

**2026-08-20 改名**：原名「请求内容拦截」有误导——它不是挡掉请求，客户端收到的是**正常的 200 应答**，只是内容由本地算出或取自配置。只改了中英显示文案，后端 settings key 仍为 `request_intercept_*`、代码标识符仍为 `requestIntercept*`，改这些要数据迁移，不值当。
...
**选号资格判定改为同源返回原因**：`service/openai_gateway_scheduling.go` 原本的
`isOpenAICompatibleAccountEligibleForRequestBeforeProfit` 只返回 bool，现改为调用
`openAICompatibleAccountEligibilityReason`，让生产判定与诊断日志走同一条路径。
2026-08-21 排查 realtime 503 时，只有布尔值的日志无法定位；补出 `elig_reason=capability_mismatch`
才锁定根因（调度快照丢 `openai_capabilities`）。**别为了"精简"把它改回只返回 bool。**
...
`handler/group_model_pricing.go`，并向两个 handler（含 `handler/api_key_handler.go`）
注入既有 `ModelPricingResolver`；
