...
v0.2.3 采用上游 `236_group_model_allowlist_repair.sql` 修复旧列残留或新列缺失，
不恢复旧的仅展示语义。Ollama 鉴权、输出限制与账号模型名称修复同样采用上游实现，
...
- 到账行只展示倍率后的基础额度；档位赠送单独显示成徽章，中文 `+$2.99+送`，英文 `Extra $2.99`。底部「支持」品牌行居中。
