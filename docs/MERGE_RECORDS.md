...
小修 `18cb70d5c`）合入 `request-audit`（`52d50eac3`），再同步上游
`98d86915b`（v0.2.4）。模型广场按用户确认保留现有价格数字，只修复不兼容单位
的官方对比、限定输入价比较、补充基础展示说明和 CI 测试登记。详见功能清单第 17 节。

上游相对 v0.2.3 更新 70 个提交、266 个文件（+5,696 / -726），采用 MiniMax 平台、
Image 2.5 与 OAuth 生图主控模型修复、HTTP/2 保活、代理编辑与备用代理修复、
Grok 媒体资格管理、模型广场有效订阅可见性、支付帮助 Markdown 与注册开关等实现。

4 处文本冲突：OpenAI 默认模型表保留 Realtime 并加入 Image 2.5；公开设置注入采用
上游排行榜开关并保留签到；登录页采用上游注册入口条件、继续从共享设置缓存读取；
自定义页面保留外链打开模式并采用上游拖动按钮。渠道缓存 pub/sub 构造器与装配同步，
Realtime 调度快照的 `openai_capabilities` 保留。支付确认、到账快照及独有媒体存储未删除。

新增迁移 `237_add_minimax_platform.sql` 扩充平台 CHECK 约束，无二开迁移文件名冲突，
不删除用户、余额或订单记录。Go 与三个 Dockerfile 同为 1.27.0。本次不部署或重启生产。
本机仅做静态检查和 gofmt，构建、单元/集成测试、前端与 lint 交由合并提交的 GitHub CI。

## 2026-09-09：合并充值档位及支付审查修复

将 `cursor/fix-recharge-credit-e227`（`4229d4182`，包含原充值页分支及后续修复）
合入 `request-audit`，自动合并无冲突。采用服务端「充值金额 × 倍率 + 档位赠送」
计算及订单到账额快照，确认框先选支付方式、刷新币种与金额预览，再独立确认下单。
保留既有回调验签、金额校验、履约幂等及退款管线；本次无数据库迁移。
功能配置、资金边界和回归用例见 `FORK_FEATURES.md` 第 16 节。

源提交的 GitHub Actions `34331416935` 已通过 test（unit + integration）、
golangci-lint、shell、frontend；合并提交的 CI 与生产分支镜像由推送后另行验证。
本机不执行构建或测试，本次不部署、不重启生产容器。

## 2026-09-08：合并上游 v0.2.3

上游从 `b7dba6267` 推进到 `772a0382f079676983c06f24b0d41e09139a8462`
（v0.2.3），9 个提交、22 个变更文件（+1,517 / -24 行）。Git fetch 与 GitHub
API 的 main SHA 一致。采用上游 Ollama Cloud 的 Anthropic Bearer 鉴权、Messages
URL 归一化及跨协议 DeepSeek 输出长度限制，并补齐账号测试模型的显示名称。

自动合并无冲突。唯一与二开共同修改的生产文件为
`backend/internal/service/account_test_service.go`：保留上游模型名称补全和四参数
鉴权 helper 调用，同时保留独有 OpenAI Realtime 探测与 GPT Image 测试兼容。
其余上游变更原样采用，没有新增与二开重复的功能；合并后相对上游为 235 个差异文件。

新增迁移 `236_group_model_allowlist_repair.sql` 修复 235 已登记但实际缺列的数据库：
只有旧列时重命名；两列并存且新列为默认空值时回填旧配置；两列均无时补建新列；
最终确保新列非空及默认值。既有非空新配置不覆盖，不删除业务记录，也不改变
v0.2.2 的白名单准入语义。迁移后仍不可不处理 schema 就直接回滚到使用旧列名的镜像。

本机仅做差异、调用签名与格式静态核验，不执行构建或测试。完整测试、lint、安全
扫描与镜像构建交由 GitHub Actions；本次不部署、不重启生产容器。
