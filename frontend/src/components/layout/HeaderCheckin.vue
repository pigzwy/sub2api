<template>
  ...
  >
    ...
    >
      ...
      <span class="hidden text-sm font-medium sm:inline">{{ t('checkin.title') }}</span>
    ...
    <transition name="dropdown">
      <div
        v-if="open"
        class="absolute right-0 top-full z-50 mt-2 w-64 rounded-lg border border-gray-200 bg-white p-3 shadow-lg dark:border-dark-700 dark:bg-dark-800"
      >
        <div class="flex items-center justify-between">
          <span class="text-sm font-semibold text-gray-900 dark:text-white">{{ t('checkin.title') }}</span>
          <span
          ...
          >
            {{ snapshot.signed_today ? t('checkin.alreadySigned') : t('checkin.badgeAvailable') }}
          </span>
        </div>
        ...
        <div class="mt-3 space-y-1.5 border-t border-gray-100 pt-2.5 text-xs dark:border-dark-700">
          <div class="flex items-center justify-between">
            <span class="text-gray-500 dark:text-dark-400">{{ t('checkin.totalDays') }}</span>
            <span class="font-medium text-gray-900 dark:text-white">{{ snapshot.total_days }}</span>
          </div>
          <div class="flex items-center justify-between">
            <span class="text-gray-500 dark:text-dark-400">{{ t('checkin.totalAmount') }}</span>
