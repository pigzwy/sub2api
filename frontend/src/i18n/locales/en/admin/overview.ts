export default {
    ...
    backup: {
      ...
      objectStorage: {
        ...
        description: 'Archives images, videos and TTS audio to S3-compatible object storage. All three share one storage target and each has its own switch and key prefix. Changes take effect on save without a restart.',
        target: 'Storage target (shared by all three)',
        reuseBackupS3: 'Reuse the backup S3 configuration above (different bucket/prefix only)',
        bucket: 'Bucket',
        bucketInherited: 'Leave empty to use the backup bucket',
        types: 'What to archive',
        imageTitle: 'Images (async generation)',
        ...
        imageOwnTarget: 'Images go to a different bucket (fill in a separate target below)',
        imageOwnTargetHint: 'When unchecked, images use the same bucket and credentials as videos and audio.',
        videoTitle: 'Videos (Grok video tasks)',
        ...
        testShared: 'Test shared target',
        testImage: 'Test image target',
        testHint: '"Test shared target" writes and deletes a probe object under the video prefix to verify the bucket and its read-write permission for real.',
