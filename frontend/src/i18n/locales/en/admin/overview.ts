export default {
    ...
    backup: {
      ...
      objectStorage: {
        ...
        description: 'Archives images, videos and TTS audio to S3-compatible object storage. All three share one set of credentials; each has its own bucket and key prefix. Changes take effect on save without a restart.',
        credentials: 'Credentials (shared by all three)',
        reuseBackupS3: 'Reuse the backup S3 configuration above (endpoint and keys come from the backup card)',
        bucket: 'Bucket',
        bucketInherited: 'Leave empty to use the backup bucket',
        types: 'What to archive',
        imageEnabled: 'Enable image offload',
        imagePrefix: 'Image key prefix',
        imagePublicBaseUrl: 'Public base URL',
        imagePublicBaseUrlPlaceholder: 'Leave empty to return presigned links',
        imagePresignExpiryHours: 'Image presigned link TTL (hours)',
        imageOwnCredentials: 'Images use a different endpoint and keys (only for a separate S3 account)',
        imageOwnCredentialsHint: 'When unchecked, images use the credentials above and only write to their own bucket.',
        ...
        videoPresignExpiryHours: 'Video presigned link TTL (hours)',
        audioEnabled: 'Enable audio offload',
        audioPrefix: 'Audio key prefix',
        audioBucketHint: 'Audio shares one config with video, so it uses the video bucket and can only have its own key prefix.',
        ...
        testShared: 'Test connection',
        testImage: 'Test image credentials',
        testHint: '"Test connection" uses the credentials above to write and delete a probe object under the video prefix in the video bucket, verifying the credentials and read-write permission for real.',
