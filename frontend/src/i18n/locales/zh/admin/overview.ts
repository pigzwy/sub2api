export default {
    ...
    backup: {
      ...
      objectStorage: {
        title: '对象存储',
        description: '把图片、视频、TTS 音频归档到 S3 兼容对象存储。三种类型共用一套凭证，各自的存储桶与 Key 前缀独立；保存后立即生效，无需重启。',
        credentials: '凭证（三种类型共用）',
        reuseBackupS3: '复用上方备份的 S3 配置（端点与密钥沿用备份卡）',
        bucket: '存储桶',
        bucketInherited: '留空则沿用备份存储桶',
        types: '要归档的类型',
        imageEnabled: '启用图片转存',
        imagePrefix: '图片 Key 前缀',
        imagePublicBaseUrl: '公开访问域名',
        imagePublicBaseUrlPlaceholder: '留空则返回预签名临时链接',
        imagePresignExpiryHours: '图片预签名有效期（小时）',
        imageOwnCredentials: '图片使用独立的端点与密钥（指向另一个 S3 账号时才勾选）',
        imageOwnCredentialsHint: '不勾选时，图片沿用上面那套凭证，只是写进自己的存储桶。',
        videoEnabled: '启用视频转存',
        videoPrefix: '视频 Key 前缀',
        videoMaxDownloadBytes: '视频转存上限（字节）',
        videoMaxDownloadBytesHint: '超过此大小的视频保留原透传路径。默认 536870912（512 MiB）。',
        videoPresignExpiryHours: '视频预签名有效期（小时）',
        audioEnabled: '启用音频转存',
        audioPrefix: '音频 Key 前缀',
        audioBucketHint: '音频与视频同属一份配置，共用视频的存储桶，只能有独立的 Key 前缀。',
        audioHint: '开启后 /v1/tts 返回的音频会异步归档为 <前缀>年/月/日/<请求 ID>.<扩展名>。归档失败只记日志，不影响接口响应与计费。',
        testShared: '测试连接',
        testImage: '测试图片凭证',
        testHint: '「测试连接」用上面那套凭证，在视频存储桶的视频前缀下写入并删除一个测试对象，真实校验凭证与读写权限。',
        saved: '对象存储配置已保存',
        testErrors: {
