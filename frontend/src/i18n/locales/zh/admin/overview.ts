export default {
    ...
    backup: {
      ...
      objectStorage: {
        title: '对象存储',
        description: '把图片、视频、TTS 音频归档到 S3 兼容对象存储。三种类型共用一套存储目标，各自独立开关与 Key 前缀；保存后立即生效，无需重启。',
        target: '存储目标（三种类型共用）',
        reuseBackupS3: '复用上方备份的 S3 配置（只用不同的存储桶/前缀）',
        bucket: '存储桶',
        bucketInherited: '留空则沿用备份存储桶',
        ...
        imageOwnTarget: '图片写入另一个桶（勾选后单独填写图片的存储目标）',
        imageOwnTargetHint: '不勾选时，图片与视频、音频写入上面同一套桶和凭证。',
        videoTitle: '视频（Grok 视频任务）',
        videoEnabled: '启用视频转存',
        ...
        testShared: '测试共用目标',
        testImage: '测试图片目标',
        testHint: '「测试共用目标」会在视频前缀下写入并删除一个测试对象，真实校验存储桶与读写权限。',
        saved: '对象存储配置已保存',
        testErrors: {
