    ...
    settings: {
      ...
      features: {
        ...
        modelPlaza: {
          ...
          enabled: '启用模型广场',
          enabledHint: '开启后侧边栏显示入口；未登录仍可通过 /model-plaza 独立访问。',
      ...
      customMenu: {
        title: '自定义菜单页面',
        description: '添加自定义页面到侧边栏导航。每个页面可以设置为普通用户或管理员可见，并选择在站内嵌入还是直接跳转。',
        itemLabel: '菜单项 #{n}',
        name: '菜单名称',
        ...
        openMode: '打开方式',
        openModeIframe: '在 iframe 中打开（嵌入本站）',
      ...
      payment: {
        ...
        balanceRechargeMultiplierHint: '用户每支付 1 单位可获得多少 USD 余额。该换算始终生效，档位赠送是额外叠加的 USD。',
        balanceRechargePreview: '预览：1 CNY = {usd} USD',
        rechargePackages: '充值档位',
        rechargePackagesHint: '控制用户充值页卡片。到账 USD = 实付金额 × 充值倍率 + 该档位赠送的 USD。赠送为 0 的档位仍按倍率入账。',
        rechargePackagesCount: '{count} 个档位',
        expandPackages: '展开',
        collapsePackages: '收起',
        addPackage: '添加档位',
        removePackage: '删除',
        ...
        field_paymentCurrencyHint: '默认 CNY。Stripe、Airwallex 和 Infini 可按账户支持从下拉项选择 HKD、USD 等币种；微信、支付宝、易支付仍按 CNY。Infini 按下单法币金额创建托管收银台，用户在 Infini 页面用 USDT 支付。',
        ...
        infiniWebhookHint: '请在 Infini 商户后台把以下地址配成 Webhook；订阅 order.update，这样才能收到 order.completed 和 order.expired。',
        infiniGuideSummary: 'Infini 托管收银台只需要创建订单、跳转 checkout_url、校验 Webhook。',
        infiniGuideNote: '在 Infini 开发者页创建 HMAC 密钥，保存页面上的 Webhook Secret。Infini 暂无商户主动退款接口，实例退款开关请保持关闭。',
