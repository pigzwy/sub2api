  ...
  try {
    ...
    if (form.purchase_subscription_enabled) {
      if (!form.purchase_subscription_url) {
        appStore.showError(
          `${t("admin.settings.purchase.url")}: URL is required when purchase is enabled`,
        );
        return;
      }
      if (!isValidHttpUrl(form.purchase_subscription_url)) {
        appStore.showError(
          `${t("admin.settings.purchase.url")}: must be an absolute http(s) URL (e.g. https://example.com)`,
        );
        return;
      }
