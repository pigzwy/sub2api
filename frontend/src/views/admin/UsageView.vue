...
// Detail tabs
type DetailTab = 'usage' | 'errors' | 'audit' | 'ranking'
const activeTab = ref<DetailTab>('usage')
const detailTabs = computed(() => {
  const tabs: Array<{ key: DetailTab; label: string; icon: 'document' | 'exclamationTriangle' | 'chart' }> = [
...
const switchTab = (tab: DetailTab) => {
  ...
  if (tab === 'audit') requestAuditPanelRef.value?.refreshData()
