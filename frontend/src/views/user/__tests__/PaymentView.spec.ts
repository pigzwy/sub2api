...
describe('PaymentView recharge rate preview', () => {
  ...
  it('uses a wide page shell and a compact bonus notice', async () => {
    ...
    expect(balanceCard.classes()).toEqual(expect.arrayContaining(['rounded-xl', 'shadow-card', 'w-fit', 'py-2', 'gap-3']))
    expect(balanceCard.classes()).not.toEqual(expect.arrayContaining(['rounded-full', 'rounded-2xl', 'py-2.5']))
